...
def test_forward_pass(model, data_module):
    ...
    try:
        ...
        if batch is None:
            print("✗ Could not get batch from dataloader")
            return False
...
def test_training_step(model, data_module):
    ...
    if model is None or data_module is None:
        print("✗ Cannot test training step - model or data_module is None")
        return False
    ...
    try:
        ...
        print(f"✓ Batch obtained for training step")
...
def main():
    ...
    if all([cfg is not None, data_module is not None, model is not None, forward_success, training_success]):
        print(f"\n🎉 ALL TESTS PASSED! The MVImageNet integration is working correctly.")
