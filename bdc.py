...
def get_checkpoint_info(base_dir: str, n_epochs: int = 20) -> List[Tuple[int, str, str, str]]:
    """
    CORRECTED: Get checkpoint information for the first n epochs.
    ...
    # You may need to adjust these based on your actual checkpoint locations
    possible_backbone_dirs = [
        Path(base_dir) / "selective_curriculum_mocov3" / "t60",
        Path(base_dir) / "curriculum_mocov3" / "t60",
        Path(base_dir) / "selective_jepa_curriculum_mocov3" / "t60",
    ]
    
    # Find the correct backbone directory
    backbone_dir = None
    for possible_dir in possible_backbone_dirs:
        if possible_dir.exists():
            backbone_dir = possible_dir
            break
    
    if not backbone_dir:
        print("Error: Could not find backbone checkpoint directory")
        print(f"Searched in: {[str(d) for d in possible_backbone_dirs]}")
        return []
    
    linear_base_dir = Path(base_dir) / "linear" / "selective_curriculum_mocov3_t60"
    
    # Use the config that has ALL required parameters for the backbone model
    # Check multiple possible locations
    possible_configs = [
        "configs/difficulty_eval_selective_curriculum.yaml",      # New dedicated config
        "scripts/pretrain/core50/selective_curriculum_mocov3.yaml",  # Original training config
        "scripts/linear_probe/core50/mocov3_batch_base.yaml",     # Your batch config
        "scripts/linear_probe/core50/mocov3_linear.yaml",         # Original
        "configs/linear_batch/linear_ep00.yaml",                  # Generated config
    ]
    
    config_path = None
    for possible_config in possible_configs:
        if Path(possible_config).exists():
            config_path = possible_config
            break
    
    if not config_path:
        print("Warning: No suitable config file found. Creating one with required parameters...")
        # Create the config directory and file
        config_dir = Path("configs/difficulty_eval")
        config_dir.mkdir(exist_ok=True, parents=True)
        config_path = str(config_dir / "selective_curriculum_config.yaml")
...
"""
    ...
    print(f"Using backbone directory: {backbone_dir}")
    print(f"Using linear base directory: {linear_base_dir}")
    print(f"Using config: {config_path}")
    
    for epoch in range(n_epochs):
        # Find backbone checkpoint - more flexible pattern matching
        backbone_patterns = [
            f"*-ep={epoch}-stp=0.ckpt",
            f"*-ep={epoch}-*.ckpt",
        ...
        for pattern in backbone_patterns:
            backbone_files = list(backbone_dir.glob(pattern))
            if backbone_files:
        ...
        # Find linear checkpoint - more flexible directory matching with recursive search
        possible_linear_dirs = [
            linear_base_dir / f"selective_curriculum_mocov3_t60_ep{epoch:02d}",
            linear_base_dir / f"ep{epoch:02d}",
            linear_base_dir / f"epoch_{epoch:02d}",
        ]
        
        linear_checkpoint = None
        for linear_dir in possible_linear_dirs:
            if linear_dir.exists():
                # Search recursively for .ckpt files (they're nested deeper)
                linear_files = list(linear_dir.rglob("*.ckpt"))
                if linear_files:
                    # Prefer files with "last" in the name, otherwise take first
                    last_ckpt = [f for f in linear_files if "last" in f.name]
                    if last_ckpt:
        ...
        if not linear_checkpoint:
            print(f"Warning: No linear checkpoint found for epoch {epoch}")
            continue
        
        checkpoint_info.append((epoch, backbone_checkpoint, linear_checkpoint, config_path))
